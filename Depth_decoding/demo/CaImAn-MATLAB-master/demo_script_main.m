...
num2read=500;					% user input: how many frames to read   (optional, default until the end)
...
tau = 12;                                          % std of gaussian kernel (half size of neuron) 
p = 1;
...
options = CNMFSetParms(...   
    ...
    'init_method','greedy_corr');               % ('greedy','greedy_corr','sparse_NMF','HALS') (default: 'greedy')
%% Data pre-processing
...
%% display components: Yr is input stack; A_or is all mask of cells; C_or is df/f, b2 is background

Df=plot_components_GUI(Yr,A_or,C_or,b2,f2,Cn,options);
number_cell=size(A_or,2);% cell number
[nx,ny,nz]=size(Y);% input stack
mask_cell= reshape(full(A_or),nx,ny,number_cell);
